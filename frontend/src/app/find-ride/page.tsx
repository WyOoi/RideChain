...
interface Ride {
  ...
  transactionSignature?: string; // Add transaction signature field
}
...
export default function FindRidePage() {
  ...
  useEffect(() => {
    async function fetchExchangeRate() {
      try {
        ...
        await convertRMToSOL(1);
        // Get the cached rate
        const rate = getCachedRMToSOLRate();
        setExchangeRate(rate);
        console.log("Updated RM to SOL exchange rate:", rate);
      } catch (error) {
    ...
    // Refresh exchange rate every 15 minutes
  ...
  async function handleLockOrReleasePayment(ride: Ride) {
    ...
    if (publicKey.toBase58() === ride.driver) {
      console.error("Cannot pay yourself");
      setPaymentErrorRideId(ride.id);
      setPaymentErrorMessage('You cannot pay yourself');
      return;
    }

    // Validate driver public key before proceeding
    let driverPublicKey: web3.PublicKey;
    try {
      console.log("Validating driver public key:", ride.driver);
      driverPublicKey = new web3.PublicKey(ride.driver);
    ...
    try {
      // Calculate amount in RM
      const amountRM = parseFloat(ride.price);
      
      if (isNaN(amountRM) || amountRM <= 0) {
        throw new Error(`Invalid amount: ${ride.price}. Please enter a positive number.`);
      }
      
      // Convert RM to SOL using real-time rates
      const amountSOL = await convertRMToSOL(amountRM);
      console.log(`[Payment] Converting RM${amountRM} to SOL: ${amountSOL}`);
      
      // Update exchange rate in case it changed
      setExchangeRate(getCachedRMToSOLRate());
      ...
      if (balance < amountLamports) {
        throw new Error(`Insufficient balance. You need ${amountSOL} SOL but have ${balance / LAMPORTS_PER_SOL} SOL`);
      }
      ...
      console.log(`[Payment] Transaction details: To ${driverPublicKey.toBase58()}, Amount: ${amountSOL} SOL (${amountLamports} lamports)`);
      console.log('[Payment] Calling sendTransaction via wallet adapter...');
      
      const signature = await sendTransaction(transaction, connection);
      console.log('[Payment] sendTransaction returned signature:', signature);
      ...
      console.log('[Payment] Transaction confirmed successfully:', signature);
      
      // Update ride payment status and store transaction signature
      const updatedRides = rides.map(r =>
        r.id === ride.id ? { 
          ...r, 
          paymentStatus: 'locked' as const,
  ...
  // Add new function to handle payment release/refund
  async function handleReleaseOrRefundPayment(ride: Ride, isRefund: boolean = false) {
    console.log(`Attempting to ${isRefund ? 'refund' : 'release'} payment for ride:`, ride.id);
    console.log("Connected Wallet Public Key:", publicKey?.toBase58());
    ...
    // Only the driver can release payment, and only the rider can request refund
    const isDriver = publicKey.toBase58() === ride.driver;
    if ((!isDriver && !isRefund) || (isDriver && isRefund)) {
      setPaymentErrorRideId(ride.id);
      setPaymentErrorMessage(isRefund ? 'Only the rider can request a refund' : 'Only the driver can release payment');
      return;
    }
    ...
    try {
      ...
      // Get the recipient's public key (driver for release, rider for refund)
      const recipientPublicKey = isRefund ? publicKey : new web3.PublicKey(ride.driver);

      // Create the transfer transaction
      const transaction = new Transaction().add(
        SystemProgram.transfer({
          fromPubkey: isRefund ? new web3.PublicKey(ride.driver) : publicKey,
      ...
      console.log(`[Payment] ${isRefund ? 'Refund' : 'Release'} transaction details: To ${recipientPublicKey.toBase58()}, Amount: ${amountSOL} SOL (${amountLamports} lamports)`);
      
      const signature = await sendTransaction(transaction, connection);
      console.log(`[Payment] ${isRefund ? 'Refund' : 'Release'} transaction sent with signature:`, signature);
      ...
      console.log(`[Payment] ${isRefund ? 'Refund' : 'Release'} transaction confirmed successfully:`, signature);
      ...
      console.error(`${isRefund ? 'Refund' : 'Release'} transaction failed:`, err);
      setPaymentErrorRideId(ride.id);
      let errorMessage = "An unexpected error occurred.";
  ...
  // Add helper function for Solana Explorer URL
                  ...
                  <div className="mt-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        ride.paymentStatus === 'locked' ? 'bg-yellow-100 text-yellow-800' :
                        ride.paymentStatus === 'released' ? 'bg-green-100 text-green-800' :
                        'bg-gray-100 text-gray-800'
                      }`}>
                        {ride.paymentStatus ? ride.paymentStatus.charAt(0).toUpperCase() + ride.paymentStatus.slice(1) : 'Pending'}
                      </span>
                    </div>
                    
                    {['locked', 'released'].includes(ride.paymentStatus as string) && typeof ride.transactionSignature === 'string' && ride.transactionSignature && (
                      <div className="mt-2">
                        <a
                          href={getSolanaExplorerUrl(ride.transactionSignature)}
                          target="_blank"
                  ...
                  <div className="flex justify-between items-center mt-4">
                    <p className={`text-lg font-bold ${ride.type === 'offer' ? 'text-green-600 dark:text-green-400' : 'text-blue-600 dark:text-blue-400'}`}>
                        ...
                        <span className="text-xs font-normal ml-1">
                          (~{(parseFloat(ride.price) * exchangeRate).toFixed(5)} SOL)
                        </span>
                        ...
                        {ride.type === 'offer' && publicKey && (
                          <div className="flex items-center space-x-2">
                            {ride.paymentStatus === 'locked' && (
                              <>
                                {publicKey.toBase58() === ride.driver ? (
                                  <button
                                    onClick={() => handleReleaseOrRefundPayment(ride, false)}
                                    disabled={paymentLoadingRideId === ride.id}
                                    className="py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-150 ease-in-out"
                                  >
                            ...
                            {ride.paymentStatus !== 'locked' && (
                              <button
                                ...
                                className={`py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white 
                                  ${ride.paymentStatus === 'released' ? 'bg-gray-400' : 'bg-purple-600 hover:bg-purple-700'}
                              ...
                              >
                                {paymentLoadingRideId === ride.id
                                  ...
                                  : ride.paymentStatus === 'released'
                                    ? 'Payment Complete'
                                    : `Pay ~${(parseFloat(ride.price) * exchangeRate).toFixed(5)} SOL`}
                              </button>
                            )}
                          </div>
                        )}
